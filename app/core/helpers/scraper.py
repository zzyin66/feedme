...
def scrape_news():
    ...
    for category in categories:
        ...
        for result in results:
            if NewsArticle.objects.filter(url=result.link).exists() or NewsArticle.objects.filter(title=result.title).exists():
                continue
            try:
                ...
                NewsArticle.objects.create(title=result.title, description=article.description, image=article.image_url, date=result.pubDate, url=result.link, category = category, keywords=keywords)
            except:
                pass
